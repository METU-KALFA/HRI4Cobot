...
def parallel_dist(x, curr_pose, prev_pose, r, show=False):
    ...
    # dot_prod = np.inner(np.array([0,0,1]), h_end[:3, 1]) # Z vector wrt world
    dot_prod = np.inner(np.array([0,-1,0]), h_end[:3, 0]) # Z vector wrt world
...
def parallel_dist_new(x, curr_pose, prev_pose, r):
    h_start = H_w1_w2 @ get_rotation(curr_pose[4])
    h_end = h_start @ H_w2_w3 @ get_rotation(x[0])
...
def gaze(drawer):
    ...
    res = least_squares(dist, x0, method='dogbox', bounds=(-np.pi, np.pi), args=(np.array([gaze_point]), curr_pose[3], r))
...
def gaze_vel(gaze_point, curr_pose, r):
    ...
    x0 = -3.14  # Horizontal gripper
    # x0 = 1.57  # Elbow Up 2
    res = least_squares(parallel_dist_new, x0, method='dogbox', bounds=(-1.5*np.pi, 1.5*np.pi), args=(curr_pose, prev_pose, r))
    # print(res)
    curr_pose[5] = res['x'][0]
    return curr_pose
